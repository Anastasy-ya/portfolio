...
const ContactForm = () => {
  ...
  const validate = () => {
    ...
    if (!formData.name.trim())
      newErrors.name = locale === 'ru' ? 'Введите имя' : 'Enter your name'
    if (!formData.email.trim())
      newErrors.email = locale === 'ru' ? 'Введите e-mail' : 'Enter your email'
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email))
      newErrors.email =
        locale === 'ru' ? 'Некорректный e-mail' : 'Invalid email'
    setErrors(newErrors)
